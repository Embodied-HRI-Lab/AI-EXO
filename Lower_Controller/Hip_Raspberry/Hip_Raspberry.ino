 ...
 *   5) Return left/right actual torque to Raspberry Pi at 100 Hz.
 ...
 *   A5 5A 43 crc8
 *
 * Teensy -> Raspberry Pi torque feedback, 14 bytes:
 *   A5 5A | 44 |
 *   seq:uint16 |
...
// Raspberry Pi -> Teensy CMD_TORQUE is expected to be sent by the Pi at 100 Hz.
// RX is asynchronous; Teensy parses commands immediately when bytes arrive.
...
{
  float requested = 0.0f;  // latest valid command from Raspberry Pi
