"""
100 Hz causal-TCN controller for bilateral hip exoskeleton deployment.

V3 adds an optional standalone realtime plot worker (--display) and separates
candidate TCN command from the torque actually sent to Teensy in the CSV.

This file is intentionally self-contained for policy inference. It supports
both the Unified TCN package and an optional symmetric thigh-IMU TCN package.
Hardware is opened only inside main() and never at import time. Use --dry-run
for no motor output. Runtime Up/Down adjusts assistance scale;
--invert-torque flips both final commands.
"""
...
try:
    import ctypes  # Windows global key state for runtime torque scaling.
except ImportError:  # pragma: no cover - non-Windows hosts.
    ctypes = None

try:
    import msvcrt
except ImportError:  # pragma: no cover - Windows-only stop-key helper.
    msvcrt = None
...
DEFAULT_MODEL_PATH = Path(__file__).resolve().with_name("unified_tcn_100hz_deploy.pt")
DEFAULT_PLOT_WORKER_PATH = Path(__file__).resolve().with_name("pc_nn_plot_worker.py")
...
class ScriptedTCNPolicy:
    """Thin runtime adapter for a self-contained scripted TCN deployment."""
    ...
    def __init__(
        ...
        *,
        device: str = "cpu",
        slew_limiter: bool = False,
        max_delta_nm_per_step: float | None = None,
    ) -> None:
        ...
        if not (0.0 < self.torque_scale_nm <= 10.0):
            raise ValueError("scripted TCN torque scale must be in (0, 10] Nm")
        if not (0.0 < self.mandatory_delta_nm <= self.torque_scale_nm):
            raise ValueError("scripted TCN slew metadata is invalid")

        self.slew_limiter_enabled = bool(slew_limiter)
        self.max_delta_nm_per_step = max_delta_nm_per_step
        self.history: deque[np.ndarray] = deque(maxlen=1)
        self.calls = 0
        self.valid_outputs = 0
        self.last_error = ""
        self.last_inference_time_ms = math.nan
        self.model.reset()
        # Trigger TorchScript graph setup before entering the 10 ms hardware loop.
        with torch.inference_mode():
            for _ in range(8):
                self.model(
                    torch.zeros(4, device=self.device), self.mandatory_delta_nm
                )
        self.model.reset()
    ...
    def infer(self) -> tuple[np.ndarray, np.ndarray] | None:
        ...
        self.calls += 1
        try:
            requested_delta_nm = self.mandatory_delta_nm
            if self.slew_limiter_enabled:
                if self.max_delta_nm_per_step is None:
                    raise ValueError(
                        "slew limiter enabled but max_delta_nm_per_step is not set"
                    )
                requested_delta_nm = min(
                    requested_delta_nm, float(self.max_delta_nm_per_step)
                )
            frame = torch.from_numpy(self.history[-1]).to(self.device)
            start = time.perf_counter()
            with torch.inference_mode():
                command = self.model(frame, requested_delta_nm)
    ...
    def print_startup_summary(self) -> None:
        print("=" * 108)
        print("MODEL: self-contained scripted TCN")
...
class UnifiedTCNPolicy:
    ...
    def __init__(self, model_path: Path, *, device: str = "cpu", slew_limiter: bool = False, max_delta_nm_per_step: float | None = None) -> None:
        ...
        self.previous_command_nm = np.zeros(2, dtype=np.float32)
        self.calls = 0
        self.valid_outputs = 0
        self.last_error = ""
        self.last_inference_time_ms = math.nan
    ...
    def reset(self) -> None:
        self.history.clear()
        self.previous_command_nm.fill(0.0)
        self.last_inference_time_ms = math.nan
    ...
    def infer(self) -> tuple[np.ndarray, np.ndarray] | None:
        ...
        try:
            ...
            if self.slew_limiter_enabled:
                if self.max_delta_nm_per_step is None:
                    raise ValueError("slew limiter enabled but max_delta_nm_per_step is not set")
                lo = self.previous_command_nm - float(self.max_delta_nm_per_step)
                hi = self.previous_command_nm + float(self.max_delta_nm_per_step)
                command_nm = np.clip(command_nm, lo, hi)
            command_nm = np.clip(command_nm, -self.torque_scale_nm, self.torque_scale_nm).astype(np.float32)
            self.previous_command_nm = command_nm.copy()
            self.valid_outputs += 1
            self.last_error = ""
    ...
    def print_startup_summary(self) -> None:
        ...
        print(f"SLEW LIMITER: {'ENABLED' if self.slew_limiter_enabled else 'DISABLED'} (baseline)")
...
def load_tcn_policy(
    ...
    *,
    ...
    """Load either a self-contained scripted policy or the Unified checkpoint."""
    ...
    kwargs = {
        ...
        "max_delta_nm_per_step": max_delta_nm_per_step,
    }
    ...
    if scripted is not None:
        ...
        return ScriptedTCNPolicy(resolved, scripted, metadata, **kwargs)

    payload = torch.load(resolved, map_location=device, weights_only=False)
    model_type = str(payload.get("model_type", ""))
    if model_type == "causal_tcn":
        return UnifiedTCNPolicy(resolved, **kwargs)
    raise ValueError(
        f"unsupported checkpoint model_type={model_type or 'missing'!r}: {resolved}"
    )
...
class TeensyLink(threading.Thread):
    ...
    def _parse_rx(self) -> int:
        ...
        while True:
            ...
            if sync_index < 0:
                # Preserve a trailing 0xA5 in case the 0x5A arrives in the next read.
...
def poll_torque_scale_keys(current_scale: float, step: float) -> tuple[float, bool]:
    """Windows global Up/Down control; one scale change per physical key press."""
    ...
    if up_pressed and not _prev_up_pressed:
        new_scale = round(min(MAX_TORQUE_SCALE, updated + step), 4)
        if abs(new_scale - updated) > 1e-12:
            updated = new_scale
            changed = True
    if down_pressed and not _prev_down_pressed:
        new_scale = round(max(MIN_TORQUE_SCALE, updated - step), 4)
        if abs(new_scale - updated) > 1e-12:
            updated = new_scale
            changed = True
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="100 Hz causal-TCN formal controller (hardware-safe v3 + display).")
    ...
    p.add_argument(
        "--torque-scale", type=float, default=DEFAULT_TORQUE_SCALE,
        help="Initial runtime assistance scale in [0.1, 1.0]; Windows Up/Down changes it.",
    )
    p.add_argument(
        "--torque-scale-step", type=float, default=DEFAULT_TORQUE_SCALE_STEP,
        help="Up/Down scale step (default: 0.1).",
    )
    p.add_argument(
        "--invert-torque", action="store_true",
        help="Invert BOTH final left/right TCN torque commands before the PC clamp. Default: no inversion.",
    )
    p.add_argument(
        "--max-torque", type=float, default=8.0,
        help="Final PC command clamp in Nm (default 8; allowed up to 10 to match current Teensy).",
    )
    ...
    p.add_argument("--mock-imu", action="store_true", help="Use synthetic thigh input; allowed only with --dry-run.")
    p.add_argument("--duration", type=float, default=None, help="Optional run duration in seconds.")
    p.add_argument("--csv", type=Path, default=None)
    p.add_argument(
        "--display", action="store_true",
        help="Launch the standalone realtime plot worker in a separate process.",
    )
    p.add_argument(
        "--plot-worker", type=Path, default=DEFAULT_PLOT_WORKER_PATH,
        help="Path to pc_nn_plot_worker.py (default: beside this controller).",
    )
    p.add_argument(
        "--plot-refresh-hz", type=float, default=30.0,
        help="Realtime plot refresh rate (default: 30 Hz).",
    )
    p.add_argument(
        "--plot-window-s", type=float, default=10.0,
        help="Visible realtime plot window length (default: 10 s).",
    )
    p.add_argument(
        "--plot-command-source", choices=("candidate", "sent"), default="candidate",
        help=(
            "Torque shown by the plot worker: 'candidate' = scaled/clamped TCN command "
            "even when disarmed; 'sent' = torque actually sent to Teensy (default: candidate)."
        ),
    )
    p.add_argument(
        "--slew-limiter",
        action="store_true",
        help=(
            "Optional extra TCN-output slew limiter. A checkpoint's mandatory "
            "slew limit remains active even when this flag is omitted."
        ),
    )
    p.add_argument("--max-delta-nm-per-step", type=float, default=None)
    return p


def validate_args(a: argparse.Namespace) -> None:
    if abs(a.rate - 100.0) > 1e-9:
        raise ValueError("TCN deployment requires --rate 100")
    if a.print_rate <= 0:
    ...
    if a.zero_settle < 0 or a.zero_samples <= 0 or a.zero_timeout <= 0:
        raise ValueError("zero-settle must be >=0 and zero-samples/zero-timeout must be positive")
    if not (MIN_TORQUE_SCALE <= a.torque_scale <= MAX_TORQUE_SCALE):
        raise ValueError(f"--torque-scale must be in [{MIN_TORQUE_SCALE}, {MAX_TORQUE_SCALE}]")
    if a.torque_scale_step <= 0:
        raise ValueError("--torque-scale-step must be positive")
    if a.max_torque <= 0 or a.max_torque > 10.0:
        raise ValueError("--max-torque must be in (0, 10] to match the current Teensy")
    if a.plot_refresh_hz <= 0:
        raise ValueError("--plot-refresh-hz must be positive")
    if a.plot_window_s <= 0:
        raise ValueError("--plot-window-s must be positive")
    if a.slew_limiter and a.max_delta_nm_per_step is None:
        raise ValueError("--slew-limiter requires --max-delta-nm-per-step")
    if a.max_delta_nm_per_step is not None and a.max_delta_nm_per_step <= 0:
    ...
    if not a.dry_run and not a.arm:
        print("[SAFETY] --arm not set: TCN runs, but Teensy receives zero torque.")
    if a.dry_run:
...
def main() -> None:
    ...
    policy = load_tcn_policy(
        a.model,
        slew_limiter=a.slew_limiter,
        max_delta_nm_per_step=a.max_delta_nm_per_step,
    )
    policy.print_startup_summary()

    torque_sign = -1.0 if a.invert_torque else 1.0
    torque_direction_text = "INVERTED (-1)" if a.invert_torque else "NORMAL (+1)"
    ...
    print(f"Ports: LEFT={a.left_port}, RIGHT={a.right_port}, TEENSY={a.teensy_port} @ {a.baud}")
    print(f"Runtime torque scale: {a.torque_scale:.2f} (Up/Down step={a.torque_scale_step:.2f}, range {MIN_TORQUE_SCALE:.1f}-{MAX_TORQUE_SCALE:.1f})")
    print(f"Final torque direction: {torque_direction_text}; use --invert-torque to flip both sides.")
    print(f"PC final clamp: +/-{a.max_torque:.2f} Nm")

    csv_path = a.csv.expanduser().resolve() if a.csv is not None else default_csv_path().resolve()
    csv_path.parent.mkdir(parents=True, exist_ok=True)

    stop_event = threading.Event()
    plot_process: subprocess.Popen | None = None
    plot_pipe_ok = False
    plot_warning_printed = False
    ...
    # ------------------------------------------------------------------
    # Hardware startup / zeroing
    # ------------------------------------------------------------------
    if not a.mock_imu:
        left_imu = SingleImuReader(
            name="LEFT", port=a.left_port, baud=a.baud,
            configure=not a.no_configure_imu, stop_event=stop_event,
        )
        right_imu = SingleImuReader(
            name="RIGHT", port=a.right_port, baud=a.baud,
            configure=not a.no_configure_imu, stop_event=stop_event,
        )
        left_imu.start()
        right_imu.start()
        ...
        rs, _ = right_imu.snapshot()
        fb, _ = teensy.snapshot()
        if ls is None or rs is None or (not a.dry_run and fb is None):
            stop_event.set()
            raise RuntimeError(
                "Startup data missing. "
                f"LEFT={left_imu.error or ('OK' if ls else 'no sample')}, "
                f"RIGHT={right_imu.error or ('OK' if rs else 'no sample')}, "
                f"Teensy={'DRY-RUN' if a.dry_run else (teensy.error or ('OK' if fb else 'no feedback'))}"
            )
    ...
    # ------------------------------------------------------------------
    # Optional standalone realtime plot process
    # ------------------------------------------------------------------
    if a.display:
        plot_worker_path = a.plot_worker.expanduser().resolve()
        if not plot_worker_path.is_file():
            raise FileNotFoundError(
                f"Plot worker not found: {plot_worker_path}. "
                "Place pc_nn_plot_worker.py beside this controller or pass --plot-worker PATH."
            )
        plot_cmd = [
            sys.executable,
            str(plot_worker_path),
            "--refresh-hz", str(a.plot_refresh_hz),
            "--window-s", str(a.plot_window_s),
            "--stale-warning-s", str(a.stale_warning),
            "--imu-timeout-s", str(a.imu_timeout),
            "--teensy-timeout-s", str(a.teensy_timeout),
        ]
        ...
        plot_pipe_ok = plot_process.stdin is not None
        print(
            f"[PLOT] worker started: {plot_worker_path.name} | "
            f"refresh={a.plot_refresh_hz:g} Hz | window={a.plot_window_s:g} s | "
            f"command-source={a.plot_command_source}"
        )
    ...
    try:
        with csv_path.open("w", newline="", encoding="utf-8-sig") as f:
            ...
            writer.writerow([
                ...
                "left_raw_angle_deg", "right_raw_angle_deg",
                ...
                "left_nominal_tcn_nm", "right_nominal_tcn_nm",
                "torque_scale", "torque_inverted",
                "left_command_nm", "right_command_nm",
                "left_sent_torque_nm", "right_sent_torque_nm",
                "left_age_ms", "right_age_ms", "teensy_age_ms",
                "imu_state_left", "imu_state_right", "teensy_ok",
                "history_ready", "inference_time_ms", "loop_time_ms",
                "overrun", "enabled",
            ])

            while not stop_event.is_set():
                torque_scale, scale_changed = poll_torque_scale_keys(
                    torque_scale, a.torque_scale_step
                )
                if scale_changed:
                    print(f"\n[SCALE] TCN torque scale -> {torque_scale:.2f}")
                ...
                if now >= next_tick:
                    ...
                    left_angle_rad = right_angle_rad = math.nan
                    left_vel_rad_s = right_vel_rad_s = math.nan
                    left_age = right_age = fb_age = 0.0 if a.mock_imu else math.inf
                    left_state = right_state = "MOCK" if a.mock_imu else "TIMEOUT"
                    teensy_ok = bool(a.dry_run)
                    imu_ok = False
                    ...
                    if a.mock_imu:
                        ...
                        if math.isfinite(left_age):
                            max_left_age_s = max(max_left_age_s, left_age)
                        if math.isfinite(right_age):
                            max_right_age_s = max(max_right_age_s, right_age)
                        if math.isfinite(fb_age):
                            max_teensy_age_s = max(max_teensy_age_s, fb_age)

                        left_state = sample_state(left_age, a.stale_warning, a.imu_timeout)
                        right_state = sample_state(right_age, a.stale_warning, a.imu_timeout)
                        imu_ok = (
                            left_sample is not None
                            and right_sample is not None
                            and left_age <= a.imu_timeout
                            and right_age <= a.imu_timeout
                        )
                        teensy_ok = a.dry_run or (
                            feedback is not None and fb_age <= a.teensy_timeout
                        )
                        ...
                        if imu_ok:
                            left_angle_deg = relative_x_deg(
                                left_sample.angle_x_deg,
                                left_zero.angle_x_deg,
                                a.left_imu_direction,
                            )
                            right_angle_deg = relative_x_deg(
                                right_sample.angle_x_deg,
                                right_zero.angle_x_deg,
                                a.right_imu_direction,
                            )
                            left_gyro_dps = relative_x_gyro_dps(
                                left_sample.gyro_x_dps,
                                left_zero.gyro_x_dps,
                                a.left_imu_direction,
                            )
                            right_gyro_dps = relative_x_gyro_dps(
                    ...
                    # When armed, fresh Teensy feedback is part of the control-valid condition.
                    control_inputs_ok = imu_ok and (a.dry_run or (not a.arm) or teensy_ok)
                    out = None

                    if control_inputs_ok:
                        policy.append_frame(
                            left_angle_rad, left_vel_rad_s,
                            right_angle_rad, right_vel_rad_s,
                        )
                        out = policy.infer()
                    else:
                        # Reset TCN history after a real control dropout so stale temporal
                        # context is never reused when communication recovers.
                        if prev_control_ok or policy.history:
                            policy.reset()
                    ...
                    if out is None:
                        ...
                        action, nominal_nm = out
                        laction, raction = float(action[0]), float(action[1])
                        lnominal = float(nominal_nm[0])
                        rnominal = float(nominal_nm[1])

                        # Final hardware command chain:
                        # TCN nominal Nm -> optional global inversion -> runtime scale -> clamp.
                        lcmd = float(np.clip(
                            torque_sign * lnominal * torque_scale,
                            -a.max_torque, a.max_torque,
                        ))
                        rcmd = float(np.clip(
                            torque_sign * rnominal * torque_scale,
                            -a.max_torque, a.max_torque,
                        ))
                        nn_valid = True
                    ...
                    prev_control_ok = bool(nn_valid and imu_ok and teensy_ok)

                    # Always send a frame on a real Teensy link. If any safety condition
                    # fails, actively send 0 Nm + enable=False rather than relying only
                    # on the Teensy watchdog. Keep the actually-sent torque separate
                    # from the candidate TCN command for unambiguous logging.
                    lsent = lcmd if enabled else 0.0
                    rsent = rcmd if enabled else 0.0
                    teensy.send_torque(lsent, rsent, enabled)

                    # Feed the existing standalone plot worker using its 12-field stdin
                    # protocol. By default the plot shows candidate TCN command so dry-run
                    # remains useful; --plot-command-source sent shows true hardware TX.
                    if a.display and plot_process is not None and plot_pipe_ok:
                        if plot_process.poll() is None and plot_process.stdin is not None:
                            try:
                                plot_left_cmd = lcmd if a.plot_command_source == "candidate" else lsent
                                plot_right_cmd = rcmd if a.plot_command_source == "candidate" else rsent
                                plot_left_angle_deg = (
                                    math.degrees(left_angle_rad)
                                    if math.isfinite(left_angle_rad) else math.nan
                                )
                                plot_right_angle_deg = (
                                    math.degrees(right_angle_rad)
                                    if math.isfinite(right_angle_rad) else math.nan
                                )
                                plot_process.stdin.write(
                                    f"{elapsed:.6f}\t"
                                    f"{plot_left_angle_deg:.6f}\t"
                                    f"{plot_right_angle_deg:.6f}\t"
                                    f"{plot_left_cmd:.6f}\t"
                                    f"{plot_right_cmd:.6f}\t"
                                    f"{left_actual:.6f}\t"
                                    f"{right_actual:.6f}\t"
                                    ...
                                    f"{right_age:.6f}\t"
                                    f"{fb_age:.6f}\t"
                                    f"{int(nn_valid and imu_ok and teensy_ok)}\t"
                                    f"{int(enabled)}\n"
                                )
                                plot_process.stdin.flush()
                            except (BrokenPipeError, OSError, ValueError) as exc:
                                plot_pipe_ok = False
                                if not plot_warning_printed:
                                    print(f"[PLOT] pipe closed: {type(exc).__name__}: {exc}")
                                    plot_warning_printed = True
                        else:
                            plot_pipe_ok = False
                            if not plot_warning_printed:
                                print("[PLOT] worker exited; control continues without realtime plot.")
                                plot_warning_printed = True
                    ...
                    writer.writerow([
                        f"{elapsed:.6f}", rows,
                        fmt(left_raw), fmt(right_raw),
                        fmt(left_angle_rad, 9), fmt(right_angle_rad, 9),
                        fmt(left_vel_rad_s, 9), fmt(right_vel_rad_s, 9),
                        fmt(left_actual), fmt(right_actual),
                        f"{laction:.9f}", f"{raction:.9f}",
                        f"{lnominal:.6f}", f"{rnominal:.6f}",
                        f"{torque_scale:.4f}", int(a.invert_torque),
                        f"{lcmd:.6f}", f"{rcmd:.6f}",
                        f"{lsent:.6f}", f"{rsent:.6f}",
                        fmt(left_age * 1000.0, 3),
                        fmt(right_age * 1000.0, 3),
                        fmt(fb_age * 1000.0, 3),
                        left_state, right_state, int(teensy_ok),
                        int(policy.history_ready), fmt(policy.last_inference_time_ms, 6),
                    ...
                    if tick_start >= next_print:
                        teen_state = "OK" if teensy_ok else "TIMEOUT"
                        print(
                            f"t={elapsed:7.3f}s "
                            f"IMU={left_state}/{right_state} T={teen_state} "
                            f"history={'READY' if policy.history_ready else 'FILL '} "
                            f"scale={torque_scale:.2f} dir={'INV' if a.invert_torque else 'NORM'} "
                            f"nom={lnominal:+.3f}/{rnominal:+.3f} "
                            f"cmd={lcmd:+.3f}/{rcmd:+.3f} "
                            f"sent={lsent:+.3f}/{rsent:+.3f} Nm "
                            f"act={left_actual:+.3f}/{right_actual:+.3f} "
                            f"inf={policy.last_inference_time_ms:.3f} ms "
        ...
        # Close the plot worker's stdin first so it can exit cleanly. If it
        # remains alive, terminate it after a short grace period.
        if plot_process is not None:
            try:
                if plot_process.stdin is not None and not plot_process.stdin.closed:
                    plot_process.stdin.close()
            except Exception:
                pass
            try:
                plot_process.wait(timeout=0.8)
            except Exception:
                try:
        ...
        duration = max(run_end_time - start_time, 1e-9)
        ...
        print(f"Final torque scale={torque_scale:.2f}; direction={torque_direction_text}; clamp=+/-{a.max_torque:.2f} Nm")
        if a.display:
            print(f"Realtime plot: enabled; command-source={a.plot_command_source}")
        print("CSV torque columns: *_command_nm = candidate after scale/clamp; *_sent_torque_nm = actual PC TX request (0 when disabled).")
        print(f"Teensy RX: {teensy_stats.hz:.1f} Hz, crc_errors={teensy_stats.crc_errors}")
        print(
            "Max data age: "
        ...
        if left_imu is not None and left_imu.error:
            print("LEFT IMU error:", left_imu.error)
        if right_imu is not None and right_imu.error:
